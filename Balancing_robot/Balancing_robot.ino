...
int acc_calibration_value = -8350;                            //Enter the accelerometer calibration value
...
void loop() {
  ...
  if (pid_output > 10 || pid_output < -10)pid_error_temp += pid_output * 0.020 ;
...
void getJoystickState(byte data[8])    {
  ...
  if      (joyX < -20)  received_byte |= B00000001; // turn left
  else if (joyX > 20)   received_byte |= B00000010; // turn right

  if      (joyY > 10)   received_byte |= B00001000; // move forward
  else if (joyY < -10)  received_byte |= B00000100; // move backward
  else                  received_byte |= B00001100; // balance
